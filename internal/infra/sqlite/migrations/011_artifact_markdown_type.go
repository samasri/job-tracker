...
func init() {
	sqlite.RegisterMigration(sqlite.Migration{
		Version: 11,
		Name:    "artifact_markdown_type",
		Up: `
			-- Recreate table with updated CHECK constraint to include 'markdown' type
			CREATE TABLE role_artifacts_new (
				id TEXT PRIMARY KEY,
				role_id TEXT NOT NULL REFERENCES roles(id),
				name TEXT NOT NULL,
				type TEXT NOT NULL CHECK(type IN ('pdf', 'jsonc', 'text', 'html', 'markdown')),
				path TEXT NOT NULL,
